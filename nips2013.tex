...
Throughout the project, we use the popular Theano project\cite{theano}, together with Lasagne\cite{lasagne}, numpy, scipy\cite{numpyscipy}, and NoLearn\cite{nolearn} libraries. Most of our experiments are executed using an Amazon Web Services EC2 machine with a GPU to exploit the speed gains of training with the CUDA framework.
...
For this project, an algorithm loosely inspired on MATLAB's OCR example\cite{automaticdetect} was written. Unfortunately, all attempts to leverage machine learning techiques in this parst of this algorithm have performed worse than hardcoding decisions (more on that in the results section).
